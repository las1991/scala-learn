...
import io.asdx.nacos4s.client.util.CollectionUtils._
import io.asdx.nacos4s.client.util.ConfigUtils.RichProperties
import io.asdx.nacos4s.client.util.NetworkUtils
...
class Nacos4sNamingService(val underlying: NamingService, val props: Properties) {
  val group = Option(props.getProperty("group")).getOrElse("DEFAULT_GROUP")
